...
function getTools() {
	...
	$.getJSON(tools, function (data) {
		if (search==true) {
			var reload='<div class="d-flex justify-content-center"><a href="index.html" class="btn btn-secondary">Zurück</a></div>'
			$('.reload').append(reload);
		}
		$.each(data, function (id, article) {
			var card='<div class="card col-9 col-xl-3 col-lg-3 col-md-4 col-sm-6 col-xs-6 animated fadeIn"><div class="card-body d-flex flex-column align-items-center">';
			var bald='http://matthiasbaldauf.com/swi1hs19/';
			var img='<img class="card-img-top border" src="'+bald+article.img+'" alt="Wrench">';
			var title='<h5 class= "card-title pt-3" lang="de"><b>'+article['label-de']+'</b></h5><h5 class= "card-title pt-3" lang="en"><b>'+article['label-en']+'</b></h5>';
			var price='<p class="card-text price">'+'Preis pro Tag: <b> CHF '+article['price-chf']+'0'+'</b></p>';
			//Prüfung ob Artikel vorhanden ist
			if (article.stock!==0) {
				var cart='<div class="input-group mb-3 mt-auto"><input type="number" id="qty" min="1" value="" class="form-control" required="true"><button class="btn cartbtn ml-1 mr-1 btn-primary" value="'+article.id+'"><i class="fas fa-cart-plus"></i></button>';
				...
				var cart='<div class="input-group mb-3 mt-auto"><input type="text" value="" class="form-control" disabled><button class="btn cartbtn ml-1 mr-1 btn-primary"  value="'+article.id+'" disabled> <i class="fas fa-cart-plus"></i></button>';
				var info='<button type="button" class="btn btn-danger info" data-toggle="popover" data-trigger="focus" title="Lagerbestand" data-content="Zurzeit nicht an Lager"><i class="fas fa-info-circle"></i></button></div></div>';
			}
