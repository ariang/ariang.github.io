...
$.getJSON('http://matthiasbaldauf.com/swi1hs19/tools', function(data) {
	$.each(data, function(id, article) {
		var card = '<div class="card animated fadeIn"><div class="card-body d-flex flex-column">';
		var bald = 'http://matthiasbaldauf.com/swi1hs19/';
		var img = '<img class="card-img-top" src="' + bald + article.img + '" alt="Wrench">';
		var title =
			'<h5 class= "card-title" lang="de">' +
			article['label-de'] +
			'</h5><h5 class= "card-title" lang="en">' +
			article['label-en'] +
			'</h5 >';
		...
		if (article.stock !== 0) {
			...
			var cart =
				'<div class="input-group mb-3 mt-auto"><input type="number" id="qty" min="1" value="" class="form-control" required="true"><button class="btn cartbtn btn-primary"  value="' +
				article.id +
			...
			var cart =
				'<div class="input-group mb-3 mt-auto"><input type="text" value="" class="form-control" disabled><button class="btn cartbtn btn-primary"  value="' +
				article.id +
				'" disabled> <i class="fas fa-cart-plus"></i></button>';
			var info =
				'<button type="button" class="btn btn-danger info" data-toggle="popover" data-trigger="focus" title="Lagerbestand" data-content="Zurzeit nich an Lager"><i class="fas fa-info-circle"></i></button></div>';
		}
	...
	$.getJSON('https://api.exchangeratesapi.io/latest?base=CHF', function(data) {
		...
		$('.cartlist').append(
			'<tr><td></td><td><th scope="row">Total:</td><td><b>' +
				...
				' / ' +
				new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'CHF' }).format(actualprice * exchange) +
				'</b></td><td></td>'
		);
	});
}
